...
**`valid` is not `trusted`, and you will meet this immediately.** An image generated by
`gpt-image-1` and checked minutes later returns `detected: true`, `validationState: 'valid'`,
`issuer: 'OpenAI OpCo, LLC'`, `model: 'gpt-image'` -- and **`trusted: false`** (verified
2026-08-09). The manifest is cryptographically intact; the issuer is simply not on the checker's
trusted list. So do not gate anything on `trusted` alone unless you mean it: for most uses
`detected && validationState === 'valid'` is the signal you actually want.
